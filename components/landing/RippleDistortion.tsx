...
vec2 coverUV(vec2 uv) {
  ...
  vec2 scaledSize = safe * max((uResolution / safe).x, (uResolution / safe).y);
  vec2 offset = (uResolution - scaledSize) * 0.5;
  return (uv * uResolution - offset) / scaledSize;
}
void main() {
  float amount = min(texture2D(uDisplacement, vUv).r, 0.35);
...
type RippleUniforms = {
  wave: { uRings: Uniform<number> };
  composite: {
    uTexture: Uniform<Texture>;
    uDisplacement: Uniform<Texture>;
    uResolution: Uniform<[number, number]>;
    uTextureSize: Uniform<[number, number]>;
    uTexel: Uniform<[number, number]>;
    uTint: Uniform<[number, number, number]>;
    uHighlight: Uniform<[number, number, number]>;
    uStrength: Uniform<number>;
    uSwirl: Uniform<number>;
    uDispersion: Uniform<number>;
    uGlint: Uniform<number>;
    uTintAmount: Uniform<number>;
    uGrayscale: Uniform<number>;
  };
};
...
export default function RippleDistortion({
  src = "/images/eleva-hero-mist.webp",
  brushSize = 150,
  strength = 0.2,
  swirl = 1,
  rings = 4,
  ...
  const configRef = useRef<RippleConfig>({ brushSize, spread, fade, spacing, clickStrength, trigger, enabled });
  ...
  useEffect(() => {
    ...
    } satisfies RippleUniforms["composite"];
    ...
    const onMove = (event: PointerEvent) => {
      ...
      if (point && (Math.abs(point[0] - previousX) > step || Math.abs(point[1] - previousY) > step)) {
    ...
    // The effect intentionally reinitializes only when the source or buffer quality changes.
